...
class Config:
    data_dir = ''
    image_size = 15  ### subtomogram size ###
    candidateKs = [2]  ### candidate number of clusters to test, it is also possible to set just one large K that overpartites the data
    ...
    dir_for_saving = 'clustering/disca'
    if not os.path.exists(dir_for_saving):
        os.makedirs(dir_for_saving)


    # model_path = '{}/model_torch.pth'.format(dir_for_saving)  ### path for saving model, should be a .pt file ###
    # label_path = '{}/labels_torch.pickle'.format(dir_for_saving)  ### path for saving labels, should be a .pkl file ###
...
def main_fn(model_name, tomogram_name):

    subtomogram_for_clustering_path = f'results/subtomograms_for_clustering/{model_name}/{tomogram_name}.npy'
    model_path = f'results/clustering/disca/{model_name}/model_torch_{tomogram_name}.pth'
    label_path = f'results/clustering/disca/{model_name}/labels_torch_{tomogram_name}.pickle'
    ## if does not exist, create a new folder
    if not os.path.exists(f'results/clustering/disca/{model_name}'):
        os.makedirs(f'results/clustering/disca/{model_name}')
    # subtomogram_for_clustering_path = f'results/subtomograms_for_clustering/Topaz/tomogram_004_bin_8.npy'
    ### Load Dataset Here ###     
    with open(subtomogram_for_clustering_path, 'rb') as f:
        data = np.load(f)
    ...
    # new_data = []
    # ## convert (50, 15, 15, 15) to (50, 32, 32, 32)
    # for i in range(data.shape[0]):
    #     new_data.append(ndimage.zoom(data[i], 32.0/15.0))
    # data = np.array(new_data)

    # zoom image
    data = ndimage.zoom(data, (1, 32.0/15.0, 32.0/15.0, 32.0/15.0))
    # data = ndimage.zoom(data, 32.0/15.0)
    ...
    ### Validate Clustering by distortion-based DBI ###
        ...
        if DBI < DBI_best:
            if i > 1:
                torch.save(model, model_path)  ### save model here ###
...
if __name__ == '__main__':
    main_fn('Topaz', 'tomogram_004_bin_8')